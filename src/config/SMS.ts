...
export async function sendSMS({ phone, message }: SMSParams): Promise<string> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `SMS request failed with status ${response.status}: ${text}`
      );
    }
...
export async function sendSMSWithPost({ phone, message }: SMSParams): Promise<string> {
