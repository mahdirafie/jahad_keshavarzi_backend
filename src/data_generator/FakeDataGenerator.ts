...
export class FakeDataGenerator {
  static async generateTractorLogs(tractorId: number, daysBack: number = 365) {
    const logs = [];
    const now = new Date();
    ...
    for (let day = daysBack; day >= 0; day--) {
      ...
      for (let hour = 0; hour < 24; hour++) {
        ...
        if (logs.length >= (day * logsPerDay) + logsPerDay) break;
      }
    }
  ...
  static async generateDataForAllTractors(tractorIds: number[]) {
  ...
  static async generateRecentData(tractorId: number) {
    const now = new Date();
    const logs = [];
